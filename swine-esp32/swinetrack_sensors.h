...
inline bool initBME680(Adafruit_BME680& bme) {
  ...
  if (ok) addrUsed = 0x76;
  if (!ok) { ok = bme.begin(0x77, &Wire); if (ok) addrUsed = 0x77; }
...
inline bool initMLX90640(Adafruit_MLX90640& mlx) {
  if (!mlx.begin(0x33, &Wire)) { Serial.println("MLX90640 not found @ 0x33"); return false; }
...
inline String makeThermalJson(const float* frame, float& tMin, float& tMax, float& tAvg) {
  tMin =  1e9; tMax = -1e9; double sum = 0;
  for (int i=0;i<32*24;i++){ float v = frame[i]; if (v<tMin) tMin=v; if (v>tMax) tMax=v; sum+=v; }
  tAvg = sum / (32.0*24.0);

  String json; json.reserve(6000);
  json = "{\"w\":32,\"h\":24,\"data\":[";
...
inline String makeReadingJson(float tempC, float hum, float press, float gasRes, float iaq,
  ...
  // small, dependency-free JSON (to reduce heap)
...
inline bool isAirQualityElevated(float gasRes, float &baseline) {
  ...
  float ratio = baseline > 0 ? (baseline / gasRes) : 1.0f; // lower gasRes => poorer IAQ
  return ratio > 1.5f; // tweak later
}
