...
class AsyncUploader {
 ...
 private:
  ...
  int sendRawTLS(const UploadItem& it) {
    ...
    if (!_writeAll(c, (const uint8_t*)head.c_str(), head.length())) { c.stop(); return -1; }
    if (!it.jpeg.empty() && !_writeAll(c, it.jpeg.data(), it.jpeg.size())) { c.stop(); return -1; }
    if (!_writeAll(c, (const uint8_t*)mid.c_str(), mid.length())) { c.stop(); return -1; }
    if (!_writeAll(c, (const uint8_t*)tail.c_str(), tail.length())) { c.stop(); return -1; }
