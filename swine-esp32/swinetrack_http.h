...
inline bool fetchCamera(const String& cameraUrl, std::vector<uint8_t>& outJpeg) {
  ...
  int32_t remaining = http.getSize();  // -1 if unknown
  outJpeg.clear();
  // Do NOT shrink_to_fit every time; that can fragment the heap.
  if (remaining > 0) outJpeg.reserve((size_t)remaining);
  else outJpeg.reserve(32768);
  ...
  while (http.connected() && (remaining != 0)) {
    ...
    if (avail) {
      ...
      if (millis() - lastProgress > 1200) break;
      delay(1);
    }
    yield();
  }
...
inline int _readHttpStatusAndDrain(WiFiClient& c, uint32_t timeoutMs = 60000) {
  ...
  auto parse = [&](const String& l) {
    ...
    return (a >= 0 && b > a) ? l.substring(a + 1, b).toInt() : -1;
  };
  auto drainHdr = [&]() {
    for (;;) {
      String h = readLine();
  ...
  while (millis() - t0 < 200) {
    ...
    while (a > 0) {
      int r = c.read(tmp, (a > (int)sizeof(tmp) ? (int)sizeof(tmp) : a));
      if (r <= 0) break;
      a -= r;
    }
  }
...
// ---------- POST multipart (HTTPClient path used by uploader) ----------
inline bool postMultipart(const char* fnBase,
  ...
  // Kept for reference if you still call this elsewhere.
  // ...
  return true;  // stub to shorten; your project can remove this if unused here
}
...
inline bool postPing(const char* fnBase) {
  ...
  if (!c.connect(host.c_str(), 443)) {
    Serial.println("[PING] TLS connect failed");
    return false;
  }
